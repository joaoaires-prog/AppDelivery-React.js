...
export default function PicaPau() {
  const [produtos, setProdutos] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  const RESTAURANTE_ID = 1; // ID para Pica Pau
    ...
    return (
      <div className="min-h-screen flex flex-col justify-center items-center lg:px-32 px-5">
        <div className="text-center">
          ...
          <p className="mt-2 text-gray-600">Carregando cardápio do Pica Pau...</p>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="min-h-screen flex flex-col justify-center items-center lg:px-32 px-5 text-red-600">
        <p>Erro ao carregar cardápio do Pica Pau: {error}</p>
