...
export default function AdminIndex() {
  ...
  useEffect(() => {
    console.log(
      "AdminIndex: useEffect chamado. User:",
      user,
      "Loading:",
      loading
    ); // LOG
    if (!loading && user && user.restaurante) {
      console.log(
        `AdminIndex: Redirecionando para /admin/editar?restaurante=${user.restaurante}`
      ); // LOG
      navigate(`/admin/editar?restaurante=${user.restaurante}`);
    } else if (!loading && !user) {
      console.log(
        "AdminIndex: Usuário não logado após carregamento ou sem restaurante, redirecionando para /login."
      ); // LOG
      navigate("/login");
    } else if (loading) {
  ...
  return (
    <div className="text-center py-12">
      ...
      <p className="text-gray-600 mt-2">
        Seu restaurante é: {user.restaurante || "Não definido"}
      </p>
      <p className="text-gray-600 mt-2">
        Aguarde o redirecionamento ou navegue para "Gerenciar Produtos".
      </p>
    </div>
  );
}
